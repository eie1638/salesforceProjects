$event.get(function(record) {
	...
	var currIPkey=null;
	currIPkey = parsedRecord.currentRec['Installed_Product__c'].key;
	if(!currIPkey){
		currIPkey = parsedRecord.currentRec['Installed_Product__c'].fieldvalue.key;
		...
		$db.get(query_object, function(query_object, response) {
			...
			if (response.results) {
				...
				$response({
					status: 'error',
					error: '',
					error_message: 'db api is successfully invalied '
				});
